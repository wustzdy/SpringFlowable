...
import static com.wustzdy.springboot.flowable.demo.constant.FlowConstants.FLOW_STAT_REVOCATION;
...
public class CustomProcessDiagramGenerator {
    ...
    private static Color STOP_COLOR = Color.GRAY;
    private static Stroke THICK_BORDER_STROKE = new BasicStroke(3f);
     ...
     * @param activityType 节点类型
     */
    private static void signStopNode(BufferedImage image, int x, int y, int width, int height, String activityType) {
        Color nodeColor = STOP_COLOR;
        Graphics2D graphics = image.createGraphics();
...
//        graphics.setStroke(THICK_BORDER_STROKE);
//        // 开始、结束节点画圆
//        if (eventType.contains(activityType)) {
...
//        this.minY -= 5;
        ...
        ProcessDefinitionEntity definition = new FindProcessDefinitionEntityCmd(processDefinitionId).execute(Context.getCommandContext());
        ...
        List<String> repeatHis = new ArrayList<>();
        String endActId = historicProcessInstance.getEndActivityId();
        ...
        for (HistoricActivityInstance historicActivityInstance : activityInstances) {
            ...
            if (activity != null) {
                ...
                Execution execution = runtimeService.createExecutionQuery().processInstanceId(processInstanceId).activityId(historicActivityId)
                        .singleResult(); // 默认查询ACT_RU_EXECUTION表中is_active=1的数据

                if (repeatHis.contains(historicActivityId)) {
                    continue;
                } else {
                    repeatHis.add(historicActivityId);
                }
                // 节点正在运行中
                if (execution != null) {
                    signRunningNode(image, activity.getX() - this.minX, activity.getY() - this.minY, activity.getWidth(), activity.getHeight(),
                            historicActivityInstance.getActivityType());
                } else if (historicActivityId.equals(endActId) && FLOW_STAT_REVOCATION.equals(deleteReason)) {
                    // 撤单节点
                    signStopNode(image, activity.getX() - this.minX, activity.getY() - this.minY, activity.getWidth(), activity.getHeight(),
                            historicActivityInstance.getActivityType());
                } else {
                    // 节点已经结束
                    signHistoryNode(image, activity.getX() - this.minX, activity.getY() - this.minY, activity.getWidth(), activity.getHeight(),
                            historicActivityInstance.getActivityType());
                }
            }
        }
